...
public class LunchRoom
{
   ...
   public static void main(String[] args)
   {
      ...
      int[] totalCustomers = new int[]{0,0,0,0};
      int[] totalWait = new int[]{0,0,0,0};
      int[] longestWait = new int[]{0,0,0,0};
      String[] classes = {"Freshman		", "Sophomore	", "Junior		", "Senior		"};
      
      int wait = (int) (Math.random() * 6) + 2;
      
      MyPriorityQueue customers = new MyPriorityQueue();
            
      for (int i = 0; i < TIME; i++) {
         if (Math.random() < 0.2)
            customers.add(new Customer(i, (int)(Math.random() * 4)));
                 
         if (wait == 0) {
            Customer student =  (Customer)customers.remove(); 
            if (i - student.getArrival() > longestWait[student.getClassNum()])
               longestWait[student.getClassNum()] = i - student.getArrival();
            totalWait[student.getClassNum()] += i - student.getArrival();
            wait = (int) (Math.random() * 6) + 2;
            totalCustomers[student.getClassNum()]++;
         } else
      ...
      /* report the data  */
      for (int i = 3; i >= 0; i--)
         System.out.printf("%s%d\t\t\t%d\t\t\t\t%f%n", classes[i], totalCustomers[i], longestWait[i], (double)totalWait[i] / totalCustomers[i]);

   
   }
   
   public static void display(int t, MyPriorityQueue q)
   {
...
{
   private int arrivalTime, classNum;
   private String[] classes = {"Fr", "So", "Jr", "Sr"};
   
   public Customer (int myTime, int myClass) {
      arrivalTime = myTime;
      classNum = myClass;
   }
   ...
   public int compareTo (Customer other) {
      if (classNum > other.getClassNum())
         return Integer.MIN_VALUE;
      else if (classNum < other.getClassNum())
         return Integer.MAX_VALUE;
      else
         return arrivalTime - other.getArrival();
   }
   
   public String toString() {
      return classes[classNum] + ": " + arrivalTime;
   }
}
